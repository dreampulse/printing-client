context('Share cart flow', () => {
  it('runs', () => {
    ...
    // create a shareable cart link and visit the cart url
    cy.contains('a', 'Share quote').click()
    cy.contains('.Headline', 'Share quote')

    cy.get('#share-cart-input').then(input => {
      cy.clearLocalStorage()
      cy.visit(
        `${input.val()}&feature:invoice&invoice_key=golden-reduce-heft-alia-cumin&utm_source=test`
      )
